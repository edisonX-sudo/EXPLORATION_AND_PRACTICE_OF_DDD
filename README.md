...
处于不同的环境,让pizza得到了完全不同的意义,同时他也能行使不同的职责(被吃,被扔).上下文可以用图片描述(上图),可以被语言描述(谁在什么地方做了什么),也可以被代码描述.从图上的角度看待,上图的pizza是处于不同上下文的,其对应的职责也是被恰当的分割到了2个上下文里.上下文内部概念及其职责都与上下文本身相关,概念复杂度被合适的限定到对应的上下文内,而上下文也能更好的被理解.
